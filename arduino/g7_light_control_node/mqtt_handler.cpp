/*
 * Wasabi SmartFarm - MQTT 핸들러 구현 (조명 제어 노드용)
 * 
 * 작성자: 서준원
 * 버전: v1.0.0
 * 날짜: 2026-02-25
...
// 생성자
// ============================================
MQTTHandler::MQTTHandler(LightControl* lightControl) 
  : _mqttClient(_wifiClient), _lastReconnectAttempt(0), _lightControl(lightControl) {
  // MQTT 클라이언트 ID 생성
...
void MQTTHandler::mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // 명령 처리 (조명 제어)
  if (_lightControl != nullptr) {
    _lightControl->handleCommand(command.c_str());
  }
}
...
bool MQTTHandler::publishState() {
  ...
  LightState state = _lightControl->getState();
  doc["node_id"] = NODE_ID;
  doc["light"] = state.light_on ? "on" : "off";
  ...
  if (success) {
    DEBUG_PRINTLN(F("[MQTT] Light state published"));
  }
