...
MQTTHandler::MQTTHandler() : _mqttClient(_wifiClient) {
    ...
    _clientId = String(MQTT_CLIENT_ID_PREFIX) + String(TANK_ID);
    
    // MQTT Topic 생성
    _dataTopic = String(MQTT_TOPIC_PREFIX) + String(TANK_ID) + String(MQTT_TOPIC_SUFFIX);
    _heartbeatTopic = String(MQTT_TOPIC_PREFIX) + String(TANK_ID) + String(MQTT_HEARTBEAT_SUFFIX);
