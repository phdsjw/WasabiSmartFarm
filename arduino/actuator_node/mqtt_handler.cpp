...
bool MQTTHandler::connectMQTT() {
  ...
  if (_mqttClient.connect(MQTT_CLIENT_ID)) {
    ...
    _mqttClient.subscribe(MQTT_TOPIC_IRRIGATION_ON);
    _mqttClient.subscribe(MQTT_TOPIC_IRRIGATION_OFF);
    _mqttClient.subscribe(MQTT_TOPIC_DRAINAGE_ON);
    _mqttClient.subscribe(MQTT_TOPIC_DRAINAGE_OFF);
    _mqttClient.subscribe(MQTT_TOPIC_FAN_ON);
    _mqttClient.subscribe(MQTT_TOPIC_FAN_OFF);
...
void MQTTHandler::handleCommand(const char* topic, const char* message) {
  // 관수 펌프 ON
  if (strcmp(topic, MQTT_TOPIC_IRRIGATION_ON) == 0) {
  ...
  // 관수 펌프 OFF
  else if (strcmp(topic, MQTT_TOPIC_IRRIGATION_OFF) == 0) {
    if (_actuatorControl->stopIrrigationPump()) {
      publishStatus("irrigation_pump_stopped");
    }
  }
  // 배수 펌프 ON
  else if (strcmp(topic, MQTT_TOPIC_DRAINAGE_ON) == 0) {
