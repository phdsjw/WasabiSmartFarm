...
MQTTHandler::MQTTHandler() 
  ...
  // MQTT 클라이언트 ID 생성
  _clientId = String(MQTT_CLIENT_ID_PREFIX) + String(ZONE_ID);
  
  // MQTT 토픽 생성
  _airDataTopic = String(MQTT_AIR_TOPIC_PREFIX) + String(ZONE_ID) + String(MQTT_AIR_DATA_SUFFIX);
  _soilDataTopic = String(MQTT_SOIL_TOPIC_PREFIX) + String(TANK_ID) + String(MQTT_SOIL_DATA_SUFFIX);
  _heartbeatTopic = String(MQTT_HEARTBEAT_PREFIX) + String(ZONE_ID) + String(MQTT_HEARTBEAT_SUFFIX);
  _statusTopic = String(MQTT_STATUS_PREFIX) + String(ZONE_ID) + String(MQTT_STATUS_SUFFIX);
}
