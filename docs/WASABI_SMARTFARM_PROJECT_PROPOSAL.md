...
│  │  - 19개 Arduino 통신  │  │  - Google Sheets (Backup) │  │
│  └────────────┬──────────┘  └───────────────────────────┘  │
└───────────────┼────────────────────────────────────────────┘
                │ MQTT (WiFi)
                │
    ...
    │           │                                   │
┌───▼───────┐ ┌─▼──────────┐   ┌──────────────┐ ┌─▼──────────┐
│센서 노드 1│ │센서 노드 2 │...│센서 노드 17  │ │센서 노드 18│
│(Tank 01)  │ │(Tank 02)   │   │(Tank 17)     │ │(Tank 18)   │
│Arduino R4 │ │Arduino R4  │   │Arduino R4    │ │Arduino R4  │
│+ SEN0604  │ │+ SEN0604   │   │+ SEN0604     │ │+ SEN0604   │
│토양센서   │ │토양센서    │   │토양센서      │ │토양센서    │
└───────────┘ └────────────┘   └──────────────┘ └────────────┘
        ...
        └───────┬───────┘
                ...
                ▼
        ┌───────────────┐
        │ 관수 펌프 2HP │──→ 18개 재배상 스프링 쿨러
        │ 퇴수 펌프 1HP │    (압력에 의한 자동 분사)
        └───────────────┘
```

### 2.2 계층별 역할

#### 2.2.1 센서 노드 (Arduino Uno R4 WiFi × 18개)
- **역할**: 각 재배상(Tank)의 토양 환경 모니터링
- **센서**: SEN0604 (4-in-1) - 토양 온도, 습도, EC, pH
- **데이터 수집**: 10초마다 센서 데이터 polling
- **데이터 전송**: MQTT로 JSON 형식 전송
...
#### 2.2.2 액추에이터 노드 (Arduino Uno R4 WiFi × 1개)
- **역할**: 관수/퇴수 펌프 제어
- **제어 대상**:
  - 관수 펌프 (2HP) - 전자개폐기 제어
  - 퇴수 펌프 (1HP) - 전자개폐기 제어
  - (선택) 환풍기, LED, 측창 모터
- **제어 방식**: 4채널 릴레이 → 전자개폐기 코일
- **MQTT Subscribe**: `actuator/irrigation_pump/on`, `actuator/irrigation_pump/off`
- **안전 기능**: 비상정지, 타임아웃, 상태 모니터링
...
#### 2.2.3 Server Layer (Node-RED + Database)
- **데이터 수집**: 18개 센서 노드 데이터 실시간 수신
- **평균값 계산**: 토양 온도/습도/EC/pH 평균
- **제어 알고리즘**: 평균값 기반 관수 조건 판단
  - 토양 습도 평균 ≤ 95%
  - 토양 EC 평균 ≥ 5.0 μS/cm
  - 토양 온도 평균 ≥ 22°C
- **제어 명령**: 액추에이터 노드에 MQTT 명령 전송
- **데이터 저장**: InfluxDB (시계열) + Google Sheets (백업)
- **웹 대시보드**: 18개 센서 데이터 시각화, 평균값 표시, 제어 버튼
...
### 3.1 분산 센서 노드 시스템 (18개)

각 센서 노드는 독립적인 Arduino Uno R4 WiFi 보드로 구성되며, 하나의 재배상(Tank)을 모니터링합니다.

#### 센서 노드 구성 (× 18세트)

| 구성 요소 | 모델명 | 통신 방식 | 수량(세트당) | 역할 |
|---------|-------|----------|------------|------|
| **MCU** | Arduino Uno R4 WiFi | WiFi | 1 | 센서 데이터 수집 및 MQTT 전송 |
| **RS485 확장보드** | DFR0259 | RS485 | 1 | Modbus RTU 통신 |
| **토양 센서** | SEN0604 (4-in-1) | Modbus RTU | 1 | 토양 온도/습도/EC/pH 측정 |
| **전원 공급장치** | 5V/3A | DC | 1 | Arduino 및 센서 전원 |
...
**센서 노드 특징:**
- ✅ **데이터 전송**: 10초마다 MQTT로 전송
- ✅ **MQTT Topic**: `sensor/tank{01~18}/data`
- ✅ **제어 기능**: 없음 (센서 전용)
- ✅ **독립 동작**: 센서 노드 간 독립적 동작
- ✅ **확장 용이**: 재배상 추가 시 노드 추가만으로 확장

### 3.2 액추에이터 제어 노드 (1개)

관수/퇴수 펌프를 제어하는 중앙 제어 노드입니다. 18개 센서 노드의 평균값 기반 제어 명령을 수신합니다.

#### 액추에이터 노드 구성

| 구성 요소 | 모델명 | 수량 | 역할 | 비고 |
|---------|-------|------|------|------|
| **MCU** | Arduino Uno R4 WiFi | 1 | 펌프 제어 및 MQTT 통신 | 제어 전용 |
| **4채널 릴레이** | 5V 릴레이 모듈 | 1 | 전자개폐기 코일 제어 | 5V 신호 |
| **SSR (40A)** | Solid State Relay | 2 | 신호 증폭 및 절연 | 관수/퇴수 펌프용 |
| **전자개폐기 (MC)** | LS MC-18b | 1 | 2HP 관수 펌프 제어 | 220V, 18A |
| **전자개폐기 (MC)** | LS MC-12b | 1 | 1HP 퇴수 펌프 제어 | 220V, 12A |
| **전원 공급장치** | 5V/3A | 1 | Arduino 전원 | - |

#### 제어 대상 (필수)

| 구동기 명칭 | 전원 규격 | 제어 방식 | 수량 | 역할 |
|-----------|---------|----------|-----|------|
| **관수 펌프** | 220V, 2HP | ON/OFF | 1 | 18개 재배상 일괄 관수 |
| **퇴수 펌프** | 220V, 1HP | ON/OFF | 1 | 배수 처리 |

**관수 시스템 특징:**
- ✅ **스프링 쿨러 방식**: 수압에 의한 자동 분사
- ✅ **개별 솔레노이드 없음**: 펌프 ON시 전체 재배상 일괄 관수
- ✅ **제어 단순화**: 펌프 ON/OFF만으로 제어
- ✅ **시간 기반**: 1시간당 4분 작동
...
| 구동기 명칭 | 전원 규격 | 제어 방식 | 수량 | 역할 |
|-----------|---------|----------|-----|------|
| **천장 환풍기** | 220V AC | ON/OFF | 1 | 온도 제어 |
| **측창 모터** | 220V AC | 정/역회전 | 4 | 환기 제어 |
| **LED 보광등** | DC 12V/24V | PWM Dimming | 1 | 광주기 제어 |

#### 선택 센서 (액추에이터 노드 추가 가능)

| 센서 | 모델명 | 수량 | 측정 대상 |
|------|-------|------|----------|
| **대기 온습도** | SHT30 | 1 | 재배상 온습도 |
| **수온** | DS18B20 | 1 | 물탱크 수온 |
| **물탱크 pH** | SEN0161 | 1 | 물탱크 pH |
| **물탱크 TDS** | SEN0244 | 1 | 물탱크 TDS |
| **물탱크 EC** | SEN0451 | 1 | 물탱크 EC |
...
#### 5.2.4 관수장비 제어 알고리즘 (평균값 기반)

**제어 철학**: 18개 센서 노드의 평균값을 계산하여 전체 재배상을 일괄 관수합니다.
...
function calculateAverages() {
    ...
    // 18개 센서 데이터 수집
    for (let tank = 1; tank <= 18; tank++) {
        let sensorData = flow.get(`sensor_tank${tank}_data`);
        
        if (sensorData && Date.now() - sensorData.timestamp < 60000) {
            토양습도_합계 += sensorData.soil_moisture;
            토양EC_합계 += sensorData.soil_ec;
            토양온도_합계 += sensorData.soil_temp;
            유효센서수++;
        }
    }
    
    // 평균값 계산 (최소 12개 센서 이상 유효해야 함)
    if (유효센서수 < 12) {
        node.warn(`유효 센서 부족: ${유효센서수}/18`);
        return null;
    }
    
    return {
        토양습도_평균: 토양습도_합계 / 유효센서수,
        토양EC_평균: 토양EC_합계 / 유효센서수,
        토양온도_평균: 토양온도_합계 / 유효센서수,
        유효센서수: 유효센서수
    };
}

// 관수 조건 판단 및 실행 (1분마다 확인)
setInterval(() => {
    // 평균값 계산
    let averages = calculateAverages();
    if (!averages) return;
    
    let 토양습도_평균 = averages.토양습도_평균;
    let 토양EC_평균 = averages.토양EC_평균;
    let 토양온도_평균 = averages.토양온도_평균;
    ...
    if (토양습도_평균 <= 95) {
        needIrrigation = true;
        reason.push(`토양 습도 평균 낮음 (${토양습도_평균.toFixed(1)}%)`);
    }
    
    // 조건 2: 평균 토양 EC 5.0 μS/cm 이상
    if (토양EC_평균 >= 5.0) {
        needIrrigation = true;
        reason.push(`토양 EC 평균 높음 (${토양EC_평균.toFixed(1)} μS/cm)`);
    }
